          ...
          {SERVICES.map((service, idx) => {
            ...
            return (
              <FadeIn key={service.id} delay={idx * 80} direction="up">
                <div
                  className={`group relative grid grid-cols-1 md:grid-cols-[100px_1fr] gap-6 md:gap-10 py-10 ${
                ...
                >
                  ...
                  <div>
                    <div className="flex items-center gap-4 mb-4">
                      <div className="w-10 h-10 bg-teal-subtle border border-teal-border flex items-center justify-center group-hover:bg-teal group-hover:border-teal transition-all duration-300 flex-shrink-0">
                        <Icon
                          size={18}
                          className="text-teal group-hover:text-ink transition-colors duration-300"
                        />
                      </div>
                      <h3 className="font-display text-xl md:text-2xl text-white font-semibold group-hover:text-teal/90 transition-colors duration-300">
                        {service.title}
                      </h3>
                    </div>

                    <p className="text-slate text-sm leading-relaxed max-w-xl mb-5">
                      {service.description}
                    </p>
                    ...
                    <ul className="flex flex-wrap gap-x-6 gap-y-2">
                      {service.deliverables.map((d) => (
                        <li
                          key={d}
                          className="flex items-center gap-2 text-xs text-slate-muted font-mono"
                        >
                          <span className="w-1 h-1 bg-teal/50 flex-shrink-0" />
                          {d}
                        </li>
                      ))}
                    </ul>
